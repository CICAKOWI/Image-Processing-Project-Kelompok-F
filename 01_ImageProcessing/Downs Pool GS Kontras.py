...
#USER ENTRY
dir_in = r"d:\tugas proyek img process\00_Dataset_Mentah\hasil duplikasi"
dir_out_base = r"d:\tugas proyek img process\01_ImageProcessing"

file_input = []
file_input.append("Aquarius")
file_input.append("Aries")
file_input.append("Cancer")
file_input.append("Capricorn")
file_input.append("Gemini")
file_input.append("Leo")
file_input.append("Libra")
file_input.append("Pisces")
file_input.append("Sagitarius")
file_input.append("Scorpio")
file_input.append("Taurus")
file_input.append("Virgo")
...
#PILIH MODE POOLING
print("Pilih mode pooling yang ingin digunakan.")
print("Ketik: average atau max")
mode_pooling = input("Mode pooling (average/max): ")
mode_pooling = mode_pooling.lower().strip()

if mode_pooling != "average" and mode_pooling != "max":
    print("Mode pooling salah.")
    print("Program dihentikan. Ketik hanya: average atau max")
    exit()

#Output dipisahkan berdasarkan mode pooling.
#Nama file tetap sama, misalnya aksun_ca (0)_ready.jpg.
dir_out = os.path.join(dir_out_base, mode_pooling)


#FUNCTION / LIBRARY
def baca_gambar(dir_in, file_input, ext):
    pic = plt.imread(os.path.join(dir_in, file_input + ext))
...
def binarisasi(pic_gs):
    ...
    #Objek aksara dibuat putih, background dibuat hitam.
    for i in range(0, row):
        for j in range(0, col):
            if pic_gs[i, j] < th:
                pic_bin[i, j] = 255
...
def down_pooling(pic, m, n, mode_pooling):
    ...
    #Hasil pooling dikembalikan ke bentuk binary 0 dan 255.
...
def preprocessing(dir_in, dir_out, file_input, ext, mode_pooling):
    path_input = os.path.join(dir_in, file_input + ext)

    if not os.path.exists(path_input):
        print("File tidak ditemukan:", path_input)
        return None
    ...
    #Nama file tidak diberi tambahan average/max.
    #Perbedaan hasil aman karena folder output dipisahkan.
    ...
    print(file_input, "selesai diproses dengan", mode_pooling, "pooling. Threshold =", th)
...
while file_input[i] != "nul":
    for k in range(0, j):
        ...
        hasil = preprocessing(dir_in, dir_out, nama_file, ext, mode_pooling)

        if hasil is None:
            jumlah_gagal += 1
        else:
            jumlah_berhasil += 1
            pic_ori, pic_gs, pic_bin, pic_clean, pic_crop, pic_square, pic_ready = hasil

    i += 1

print("")
print("Semua gambar selesai diproses.")
print("Mode pooling    :", mode_pooling)
print("Jumlah berhasil :", jumlah_berhasil)
print("Jumlah gagal    :", jumlah_gagal)
print("Hasil ready     :", dir_out)
print("Hasil debug     :", os.path.join(dir_out, "_debug"))


#MENAMPILKAN GAMBAR TERAKHIR YANG BERHASIL DIPROSES
if jumlah_berhasil > 0:
    ...
    plt.figure("pic_clean")
    plt.imshow(pic_clean, cmap="gray")

    plt.figure("pic_crop")
    plt.imshow(pic_crop, cmap="gray")

    plt.figure("pic_square")
    plt.imshow(pic_square, cmap="gray")

    plt.figure("pic_ready_" + mode_pooling)
    plt.imshow(pic_ready, cmap="gray")
